"""Evaluate Gemma-4-31B-it (text-only) on round 15 of Korean Bar Exam.

Loads Gemma4ForCausalLM from the multimodal Gemma4ForConditionalGeneration
checkpoint by extracting text_config and text weights. Greedy decode.
"""
...
def load_text_only(model_dir: str, device: str, dtype: torch.dtype):
    cfg = json.load(open(Path(model_dir) / 'config.json'))
    text_cfg = Gemma4TextConfig(**cfg['text_config'])
    ...
    for shard, keys in shards.items():
        data = load_file(str(Path(model_dir) / shard))
        for k in keys:
            if 'vision' in k or 'audio' in k or 'visual' in k or 'multi_modal' in k:
                continue
            new_k = k.replace('model.language_model.', 'model.')
            state_dict[new_k] = data[k]
    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    print(f'text-only load: {len(state_dict)} tensors, missing={len(missing)}, unexpected={len(unexpected)}', flush=True)
...
def main() -> None:
    ...
    print(f"loading model text-only from {args.model} ...", flush=True)
    tok = AutoTokenizer.from_pretrained(args.model, trust_remote_code=True)
    model = load_text_only(args.model, "cuda", dtype)
    ...
    with out_path.open("w", encoding="utf-8") as fpo:
        for idx, row in enumerate(rows):
            ...
            fpo.write(json.dumps({
                "idx": idx, "id": row.get("id", ""), "subject": subj,
    ...
    summary = {
        "model": args.model, "round": args.round,
        "n_total": n_total, "n_parsed": n_parsed, "n_correct": n_correct,
        "accuracy": n_correct / n_total if n_total else 0.0,
        "parse_rate": n_parsed / n_total if n_total else 0.0,
        "elapsed_seconds": time.time() - t0,
        "by_subject": {
            s: {"accuracy": v["correct"] / v["total"] if v["total"] else 0.0,
