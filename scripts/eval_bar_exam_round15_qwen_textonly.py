"""Evaluate Qwen3.5-9B (text-only) on round 15 of Korean Bar Exam.

Loads only the language_model weights from the multimodal checkpoint to skip
the vision encoder. Greedy decode, parse '정답: X'.
"""
...
def load_text_only(model_dir: str, device: str, dtype: torch.dtype):
    ...
    text_cfg = Qwen3_5TextConfig(**cfg['text_config'])
