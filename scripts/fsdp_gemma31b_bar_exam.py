"""FSDP full-parameter SFT for Gemma-4-31B-it on 4× H200.

bf16 model, FSDP shard across 4 GPUs, paged_adamw_8bit, gradient checkpointing.
Memory per GPU: ~60GB (model shard + grad shard + optimizer shard + activation).
"""
...
def main():
    ...
    try:
        ...
        for name, mod in list(model.named_modules()):
            if isinstance(mod, Gemma4ClippableLinear):
                ...
                n += 1
        print(f"unwrapped {n} ClippableLinear", flush=True)
    ...
    total_steps = int(args.epochs * len(ds) / (args.per_device_batch * args.grad_accum * 4))
    print(f"=== FSDP FULL SFT START total_steps~{total_steps} batch={args.per_device_batch}x{args.grad_accum}x4gpu ===", flush=True)
    trainer.train()
