"""FSDP full-parameter SFT for Qwen3.6-27B on 2× H200."""
from __future__ import annotations
import argparse, json
from pathlib import Path
import torch
from torch.utils.data import Dataset
from transformers import AutoTokenizer, DataCollatorForSeq2Seq, Qwen3_5ForConditionalGeneration, Trainer, TrainingArguments
...
class JsonlSFTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        row = self.rows[idx]
        messages = [{"role":"user","content":build_user(row)},{"role":"assistant","content":row["answer"]}]
...
def main():
    ...
    print(f"loading bf16 model (FSDP will shard)...", flush=True)
    model = Qwen3_5ForConditionalGeneration.from_pretrained(
        args.model, dtype=torch.bfloat16, trust_remote_code=True,
        attn_implementation="sdpa", low_cpu_mem_usage=True)
    model.config.use_cache = False
    total = sum(p.numel() for p in model.parameters()) / 1e9
    print(f"total params: {total:.2f}B (all trainable)", flush=True)
    ...
    targs = TrainingArguments(
        ...
        disable_tqdm=False, logging_first_step=True,
        remove_unused_columns=False, max_grad_norm=1.0,
        fsdp="full_shard auto_wrap",
        fsdp_config={"fsdp_transformer_layer_cls_to_wrap": ["Qwen3_5DecoderLayer"], },
    )
    trainer = Trainer(model=model, args=targs, train_dataset=ds, data_collator=collate_fn)
    total_steps = int(args.epochs * len(ds) / (args.per_device_batch * args.grad_accum * 2))
    print(f"=== FSDP FULL SFT START total_steps~{total_steps} batch={args.per_device_batch}x{args.grad_accum}x2gpu ===", flush=True)
    trainer.train()
    trainer.save_model(args.output)
    tok.save_pretrained(args.output)
    print(f"=== FSDP FULL SFT DONE saved to {args.output} ===", flush=True)
