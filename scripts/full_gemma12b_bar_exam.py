"""Full parameter SFT for Gemma-4-12B-it on Korean Bar Exam data.

12B in bf16 fits H200 (model 24GB + grad 24GB + 8-bit Adam 12GB + activation
15GB = ~75GB). No quantization needed.
"""
...
def main():
    ...
    print(f"loading bf16 model from {args.model} ...", flush=True)
    ...
    print(f"=== FULL SFT START total_steps={total_steps} batch={args.per_device_batch}x{args.grad_accum} (bf16, no LoRA) ===", flush=True)
    trainer.train()
