"""Full parameter SFT for Gemma-4-31B-it (no LoRA) on Korean Bar Exam.

Loads multimodal Gemma-4 in 4-bit NF4, unwraps ClippableLinear, and trains
ALL parameters with paged_adamw_8bit. Memory budget on H200:
  - 4-bit base: ~16GB
  - Full bf16 gradients: ~62GB
  - paged_adamw_8bit state: ~31GB
  - Activations (grad ckpt): ~15GB
  Total: ~124GB → fits H200 143GB
"""
...
def main() -> None:
    ...
    try:
        ...
        print("unwrapped ClippableLinear", flush=True)
    except Exception as e:
        print(f"unwrap skip: {e}", flush=True)

    # Enable input require grads for gradient checkpointing on 4-bit base
    ...
    print(f"params total={total:.2f}B trainable={trainable:.2f}B", flush=True)
    ...
    print(f"=== FULL SFT START total_steps={total_steps} batch={args.per_device_batch}x{args.grad_accum} (no LoRA) ===", flush=True)
    trainer.train()
    trainer.save_model(args.output)
    tok.save_pretrained(args.output)
    print(f"=== FULL SFT DONE saved to {args.output} ===", flush=True)
